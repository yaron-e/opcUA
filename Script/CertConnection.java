 ...
 */
public class CertConnection {

    private static final Logger logger = LoggerFactory.getLogger(CertConnection.class);
    ...
    public void startCertConnection() throws SecureIdentityException, ServerListException, IOException, SessionActivationException, URISyntaxException {
//        return Completable.create(emitter -> {
        try {
            ...
            System.out.println(validationListener.toString());
...
//        );
    ...
    protected MonitoredDataItemListener dataChangeListener = new MyMonitoredDataItemListener_local(this);
    protected String defaultServerAddress = "opc.tcp://10.249.128.132:4840";
    protected SecurityMode defaultSecurityMode = SecurityMode.BASIC256SHA256_SIGN_ENCRYPT;
    ...
    MonitoredEventItemListener eventListener = new MyMonitoredEventItemListener_local(this, eventFieldNames);

    protected final List<String> initialMonitoredItems = new ArrayList<String>();

    protected NodeId nodeId = null;
    protected String passWord;
    protected SecurityMode securityMode = this.defaultSecurityMode;
    ...
    public CertConnection(String filePath) {
    ...
    /**
     * Call a method on an object. Note that methods are called on objects, so you
     * need to define the NodeId for both the object and the method.
     *
     ...
     * ModelChange events, and requests the specific event fields that are defined
     * in 'eventFieldNames'.
     *
     * @return
     */
    protected EventFilter createEventFilter(QualifiedName[] eventFields) {
        ...
        /*
         * Add a field to get the NodeId of the event source, this is done by selecting
         * an empty path so it is the node itself.
         *
         * NOTE! Previously we have used null array instead of new QualifiedName[0] for
         * the browsePath selector. Most servers accept null, but not all and using
         * empty array form is more correct.
         */
        selectClauses[eventFields.length] = new SimpleAttributeOperand(eventTypeId, new QualifiedName[0],
    ...
    protected MonitoredDataItem createMonitoredDataItem(NodeId nodeId, UnsignedInteger attributeId) {
        /*
         * Creating MonitoredDataItem, could also use the constructor without the
         * sampling interval parameter, i.e. it would be default -1 and use the
         * publishing interval of the subscription, but CTT expects positive values here
         * for some tests.
         */
    ...
    /**
     * Example of discovering applications from a discovery server (LDS or GDS or
     * the server itself) and selecting an endpoint.
     *
    ...
    protected String[] discoverServer(String uri) throws ServerListException {
        ...
        if (!serverList.isEmpty()) {
            ...
            println(String.format("%s - %-40s - %-15s - %-50s - %s", "#", "Name", "Type", "ProductUri",
                    "ApplicationUri"));
        ...
        if (!serversOnNetwork.isEmpty()) {
            ...
            for (int i = 0; i < serversOnNetwork.size(); i++) {
                ...
                try {
                    ...
                    // If the capabilities is invalid, display the original as list, and the
                    // exception
     ...
     * @throws ServerListException
     */
    protected void initialize() throws URISyntaxException, SecureIdentityException, IOException,
        ...
        // IF connection is in reverse mode (cmdline argument '-r'), must set these as
        // well.
        if (isInReverseConnectionMode) {

            // You could also use setReverseAddress, if you wish to only bind on a specific
            // local address
        ...
        validator.getIgnoredChecks().add(IgnoredChecks.IGNORE_CA_MISSING_CRL);
        ...
        System.out.println("setApplicationUri() = " + appDescription.getApplicationUri());
        System.out.println("getProductUri() = " + appDescription.getProductUri());

        // *** Certificates
        logger.info("Loading certificates..");

//        File privatePath = new File(certStore.getBaseDir() + "/private/");
        File derFile = new File(
                filePath + "/private/SampleConsoleClient@xubuntu-20.04.1-desktop_2048.der");
        File pemFile = new File(
                filePath + "/private/SampleConsoleClient@xubuntu-20.04.1-desktop_2048.pem");
        logger.error("DER = " + derFile.getAbsolutePath());
        if (!derFile.exists()) {
            logger.error("DER don't exist");
        }
        if (!pemFile.exists()) {
            logger.error("PEM don't exist");
        }
...
//        "dataFEEDOpcUaClient", /* Private Key Password, optional */"", /* Key File Path */privatePath,
//        /* CA certificate & private key, optional */null,
//        /* Key Sizes for instance certificates to create, optional */keySizes,
//        /* Enable renewing the certificate */true);
        File file = new File(filePath + "/private/");
        if (!file.exists())
            System.out.println("File not found!");
        final ApplicationIdentity identity = // createApplicationIdentity(privatePath, appDescription,
                // "dataFEEDOpcUaClient");
                ApplicationIdentity.loadOrCreateCertificate(appDescription, "dataFEEDOpcUaClient",
                        /* Private Key Password, optional */ "", /* Key File Path */file,
                        /* Enable renewing the certificate */ true);
        ...
        client.setSecurityMode(this.defaultSecurityMode);
//		System.out.println("Security mode = " + SecurityMode.BASIC256SHA256_SIGN_ENCRYPT);
...
//		client.setSecurityMode(securityMode);
        ...
        if (userName == null) {
            ...
            client.setUserIdentity(new UserIdentity(derFile, pemFile, ""));
//            logger.error("!!!!!!!!path to PKI = " + privatePath);
        } else {
...
//		System.out.println("-------------------------------------------------------");
     ...
     * @throws AddressSpaceException
     * @throws ServiceException
     * @throws MethodArgumentException if the output arguments are not valid for the
     *                                 node
     * @throws StatusException
     */
...
//		println("(n=None,s=Sign,e=SignAndEncrypt)");
    ...
    protected String promptServerAddress() throws IllegalArgumentException {
        if (isInReverseConnectionMode) {
            while (true) {
                try {
//					println("Connecting in reverse connection mode. Enter a local port number on which to listen reverse connections.");
                    String listenAddressPort = defaultServerAddress;//readInput(false);
        ...
        while (true) {
            ...
            String address = defaultServerAddress;// readInput(false, defaultServerAddress);
            try {
                ...
                switch (action) {
                    ...
                    default:
                        ...
                        AggregateConfiguration aggregateConfiguration = new AggregateConfiguration(false, true,
                                UnsignedByte.valueOf(100), UnsignedByte.valueOf(100), false);
            ...
            try {
                ...
                if (events != null) {
                    ...
                    for (int i = 0; i < events.length; i++) {
                        println("Event " + (i + 1) + " = " + eventFieldsToString(eventFieldNames,
                                Variant.asVariantArray(events[i].getEventFields())));
                    }
                }
            } catch (Exception e) {
                printException(e);
        ...
        do {
            ...
            if (input.equals("r")) {
                ...
                try {
                    ...
                    /*
                     * Nevermind. Well actually the CTT does check that these exceptions are
                     * displayed so..
                     */
...
class MyMonitoredDataItemListener_local implements MonitoredDataItemListener {
    private final CertConnection client;
    private final CertConnection certClient;

    public MyMonitoredDataItemListener_local(CertConnection certConnection) {
        this.certClient = certConnection;
        this.client = null;
    }
    ...
    public void onDataChange(MonitoredDataItem sender, DataValue prevValue, DataValue value) {
        if (this.client != null)
            CertConnection.println(client.dataValueToString(sender.getNodeId(), sender.getAttributeId(), value));
        else
            CertConnection.println(certClient.dataValueToString(sender.getNodeId(), sender.getAttributeId(), value));
    }
}

class MyMonitoredEventItemListener_local implements MonitoredEventItemListener {
    private final QualifiedName[] requestedEventFields;
    private final CertConnection certConnectionClient;

    public MyMonitoredEventItemListener_local(CertConnection certConnection, QualifiedName[] eventFieldNames) {
        this.requestedEventFields = eventFieldNames;
        this.certConnectionClient = certConnection;
    }

    @Override
    public void onEvent(MonitoredEventItem sender, Variant[] eventFields) {
        CertConnection.println(certConnectionClient.eventToString(sender.getNodeId(), requestedEventFields, eventFields));
    }
}

class MyReverseConnectionListener_local implements ReverseConnectionListener {
...
class MySubscriptionAliveListener_local implements SubscriptionAliveListener {
    ...
    public void onAfterCreate(Subscription subscription) {
        ...
        CertConnection.println(String.format("%tc Subscription created: ID=%d lastAlive=%tc", Calendar.getInstance(),
    ...
    public void onAlive(Subscription subscription) {
        ...
        CertConnection.println(String.format("%tc Subscription alive: ID=%d lastAlive=%tc", Calendar.getInstance(),
                subscription.getSubscriptionId().getValue(), subscription.getLastAlive()));
    }

    @Override
    public void onLifetimeTimeout(Subscription subscription) {
        CertConnection.println(String.format("%tc Subscription lifetime ended: ID=%d lastAlive=%tc",
    ...
    public void onTimeout(Subscription subscription) {
        ...
        CertConnection.println(String.format("%tc Subscription timeout: ID=%d lastAlive=%tc", Calendar.getInstance(),
                subscription.getSubscriptionId().getValue(), subscription.getLastAlive()));
    }
}

class MySubscriptionNotificationListener_local implements SubscriptionNotificationListener {

    @Override
    public void onBufferOverflow(Subscription subscription, UnsignedInteger sequenceNumber,
                                 ExtensionObject[] notificationData) {
        CertConnection.println("*** SUBCRIPTION BUFFER OVERFLOW ***");
    }
    ...
    public void onError(Subscription subscription, Object notification, Exception exception) {
        ...
        // an EventList
        CertConnection.printException(exception);
    }
